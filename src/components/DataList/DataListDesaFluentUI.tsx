...
import { FormulirDesa } from "../Formulir/formulir-desa"; 
import find from "lodash.find";
import { useGetDaftarDataKabupatenQuery, useGetDaftarDataDesaQuery, useGetDaftarDataPropinsiQuery, useGetJumlahDataDesaQuery, useGetDaftarDataKecamatanQuery } from "../../features/repository/service/sikoling-api-slice";


interface IDataDesaFluentUIProps {
    initSelectedFilters: IQueryParamFilters;
    title?: string;
};
type IItemDesa = {key: string|null;} & Partial<IDesa>;
...
export const DataListDesaFluentUI: FC<IDataDesaFluentUIProps> = ({initSelectedFilters, title}) => {  
    ...
    const [isModalFormulirDesaOpen, { setTrue: showModalFormulirDesa, setFalse: hideModalFormulirDesa }] = useBoolean(false);
    const [isModalSelection, setIsModalSelection] = useState<boolean>(false);
    const [dataLama, setDataLama]= useState<IDesa|undefined>(undefined);
    ...
    const [columns, setColumns] = useState<IColumn[]>([    
        ...
        { 
            ...
            isPadded: true,
            onRender: (item: IItemDesa) => {
                return item.kecamatan?.kabupaten?.nama;
            }
        },
        { 
            key: 'propinsi', 
            ...
            isPadded: true,
            onRender: (item: IItemDesa) => {
                return item.kecamatan?.kabupaten?.propinsi?.nama;
            },
        },
    ]);
    // rtk hook state
    const { data: postsCount, isLoading: isLoadingCountPosts } = useGetJumlahDataDesaQuery(queryFilters);
    const { data: postsDesa, isLoading: isLoadingPostsDesa } = useGetDaftarDataDesaQuery(queryParams);
            ...
            return [
                ...
                { 
                    ...
                    onClick: () => {
                        ...
                        setModeForm('edit');
                        showModalFormulirDesa();
                        let dataTerpilih: IDesa = find(postsDesa, (i: IDesa) => i.id == selection.getSelection()[0].key) as IDesa;
        ...
        () => {
            ...
            setQueryFilters(
                prev => {
                    ...
                    found = filters?.findIndex((obj) => {return obj.fieldName == 'kecamatan'}) as number;
                    if(found != -1) {
                        filters?.splice(found, 1);  
                    }
    ...
    const _onHandleOnChangeKecamatan = useCallback(
        (event: React.FormEvent<IComboBox>, option?: IComboBoxOption, index?: number, value?: string) => {
            let item = cloneDeep(postsKecamatan?.at(index!));
    ...
    return (
        <Stack grow verticalFill>
                ...
                <Stack grow verticalFill tokens={stackTokens} className={classNames.container}>
                    <Stack.Item grow className={classNames.gridContainer}>
                        <ScrollablePane scrollbarVisibility="auto">
                            <DetailsList
                                items={
                                    postsDesa != undefined ? postsDesa?.map(
                                        (t) => (
                    ...
                    <Stack>
                        ...
                        <Stack.Item>
                            <ComboBox
                                ...
                                selectedKey={selectedKeyKecamatan == undefined ? null:selectedKeyKecamatan}
                                useComboBoxAsMenuWidth={true}     
                                onChange={_onHandleOnChangeKecamatan}
                                disabled={selectedKeyKabupaten == undefined ? true:false}
                            />
                        </Stack.Item>
                        <Stack.Item>
                            <PrimaryButton 
            ...
            { isModalFormulirDesaOpen == true ?
                <FormulirDesa
                    title={formulirTitle}
                    isModalOpen={isModalFormulirDesaOpen}
                    showModal={showModalFormulirDesa}
                    hideModal={hideModalFormulirDesa}
                    mode={modeForm}
                    dataLama={dataLama}
                />:null
            }

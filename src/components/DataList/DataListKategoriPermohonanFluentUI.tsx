...
export const DataListKategoriPermohonanFluentUI: FC<IDataListKategoriPermohonanFluentUIProps> = ({initSelectedFilters, title}) => {   
    ...
    const [isModalFormulirKategoriPermohonanOpen, { setTrue: showModalFormulirKategoriPermohonan, setFalse: hideModalFormulirKategoriPermohonan}] = useBoolean(false);
    const [isSelectedItem, setIsSelectedItem] = useState<boolean>(false);
    const [isModalSelection, setIsModalSelection] = useState<boolean>(false);
    const [dataLama, setDataLama]= useState<IKategoriPermohonan|undefined>(undefined);
    ...
    // rtk hook state
    const { data: postsCount, isLoading: isLoadingCount } = useGetJumlahDataKategoriPermohonanQuery(queryFilters);
    const { data: postsKategoriPermohonan, isLoading: isLoadingPosts } = useGetDaftarDataKategoriPermohonanQuery(queryParams);   
    ...
    const itemsBar: ICommandBarItemProps[] = useMemo(
            ...
            return [
                { 
                    ...
                    onClick: () => {
                        ...
                        setModeForm('add');
                        showModalFormulirKategoriPermohonan();
                ...
                { 
                    ...
                    onClick: () => {
                        ...
                        setModeForm('edit');
                        showModalFormulirKategoriPermohonan();                        
                        let dataTerpilih = find(postsKategoriPermohonan, (i) => i.id == selection.getSelection()[0].key);
        ...
        [isSelectedItem, selection, postsKategoriPermohonan]
    );   
